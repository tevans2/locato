...
function createPlayer(id: PlayerId, name: string): GeoGuessrPlayerState {
...
function toPublicPlayer(player: GeoGuessrPlayerState): PublicPlayerState {
  const { roundsGuessed: _guessed, roundsMissed: _missed, ...publicPlayer } = player;
  return publicPlayer;
}

export class GeoGuessrRoom {
  ...
  constructor(options: {
    ...
    this.maxPlayers = options.maxPlayers ?? DEFAULT_GEOGUESSR_MAX_PLAYERS;
  ...
  updateOptions(playerId: PlayerId, options: { readonly roundLimit?: number; readonly roundDurationMs?: number }, now: number): RoomResult {
    ...
    this.locationQueue = createGeoGuessrQueue(`${this.seed}:options`, this.roundLimit);
    for (const [id, player] of this.players) {
  ...
  finalResults(): readonly FinalResult[] {
    const sorted = [...this.players.values()].sort((left, right) => right.score - left.score || left.name.localeCompare(right.name));
  ...
  private createChatMessage(player: GeoGuessrPlayerState, text: string, now: number): PublicChatMessage {
    this.chatSequence += 1;
    return { id: `${this.code}:${now}:${this.chatSequence}`, playerId: player.id, playerName: player.name, text: filterProfanity(text), sentAt: now };
  }
